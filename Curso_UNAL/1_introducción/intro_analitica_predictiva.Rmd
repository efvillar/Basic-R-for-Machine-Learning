...
```{r}
datos_ventas_autos$mes<-strftime(datos_ventas_autos$Fecha, format = "%m")
...
```{r}
datos_ventas_autos$diaano<-strftime(datos_ventas_autos$Fecha, format = "%j")  #j se usa para dia del año
```
...
datos_ventas_autos$dia_semana<-ordered(datos_ventas_autos$dia_semana,levels=c( "lunes", "martes", "miércoles", 
...
```{r}
dias_habiles<-ifelse(!(datos_ventas_autos$dia_semana %in% c("sabado","domingo")) | 
